...
/*   Updated: 2023/11/21 19:25:10 by yhwang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_obj(t_scene *scene, char *line)
{
	...
	{
		if ((i > 0 && init_scene_obj(scene, i)) || parse_sphere(scene, i, line))
			return (1);
	}
	else if (!ft_strncmp(line, "pl", 2) && count_token(line, ' ', 4) == 4)
	{
		if ((i > 0 && init_scene_obj(scene, i)) || parse_plane(scene, i, line))
			return (1);
	}
	else if (!ft_strncmp(line, "cy", 2) && count_token(line, ' ', 6) == 6)
	{
		if ((i > 0 && init_scene_obj(scene, i))
